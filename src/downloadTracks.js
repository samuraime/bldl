  ...
  const multiBar = new cliProgress.MultiBar(
    {
      ...
      hideCursor: true,
      format: ' {bar} | {title} | {mimeType} | {transferred}/{size}',
    },
    cliProgress.Presets.shades_grey,
  );

  const downloadedTracks = tracks.map((track) => {
    const bar = multiBar.create(1, 0, {title: metadata.title, mimeType: track.mimeType, transferred: 0, size: 'Unknown'});
    ...
    return downloadTrack({
      ...
      onProgress: ({ percent, transferred, total }) => {
        bar.update(percent, { title: metadata.title, mimeType: track.mimeType, transferred, size: total });
      },
    })
